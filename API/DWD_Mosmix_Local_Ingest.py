...
import pandas as pd
import xarray as xr
import zarr.storage # Used for zarr.storage.FSStore directly if needed, but xarray handles
import requests
import io
import zipfile
from pykml import parser # For parsing KML/KMZ files
from lxml import etree # For XML tree manipulation (used by pykml implicitly)
...
def calculate_relative_humidity(T_kelvin, Td_kelvin):
    ...
    rh = np.where(
        ...
        (
            ...
            / _calculate_saturation_vapor_pressure(T_kelvin) # Saturation vapor pressure
...
def parse_mosmix_kml(kml_filepath):
    ...
    if product_definition_elem is not None:
        ...
        global_metadata["IssueTime"] = pd.to_datetime(issue_time_text) if issue_time_text else pd.NaT
    ...
    for placemark in root.findall(f".//{KML_NAMESPACE}Placemark"):
        ...
        if coordinates_elem is not None and coordinates_elem.text:
            ...
            try:
                ...
                print(f"Warning: Could not parse coordinates for station ID {station_id}: {coordinates_elem.text}. Setting to NaN.")
                lon, lat, alt = np.nan, np.nan, np.nan

        extended_data_pm = placemark.find(f"{KML_NAMESPACE}ExtendedData")
        if extended_data_pm is None:
            print(f"Warning: No ExtendedData found for station ID {station_id}. Skipping.")
            continue
        ...
        for forecast_elem in extended_data_pm.findall(f"{DWD_NAMESPACE}Forecast"):
            ...
            if element_name and value_elem is not None and value_elem.text:
                ...
                if len(param_values) == len(global_forecast_times):
                    ...
                    print(f"Warning: Mismatch in number of values for '{element_name}' for station {station_id}. Skipping parameter.")
        ...
        for i, ftime in enumerate(global_forecast_times):
            record = {"station_id": station_id, "station_name": station_name,
                      "longitude": lon, "latitude": lat, "altitude": alt, "time": ftime}
...
def convert_df_to_xarray(df, global_metadata=None):
    ...
    data_vars_cols = [col for col in df.columns if col not in ["station_id", "time", "station_name", "longitude", "latitude", "altitude"]]
    ...
    dwd_original_vars_to_drop = ["PPPP", "TX", "TTT", "Td", "FF", "DD", "FX1", "RR1c", "VV", "N", "WW", "Rad1h"]
    ...
    if "Rh" in ds.data_vars: ds = ds.drop_vars("Rh")
    if "R101" in ds.data_vars: ds = ds.drop_vars("R101")
...
def interpolate_dwd_to_grid(dwd_ds_point, gfs_lats, gfs_lons, numerical_variables_to_interpolate, all_zarr_schema_vars):
    ...
    for var_name in numerical_variables_to_interpolate:
        if var_name in dwd_ds_point.data_vars:
            ...
            gridded_data_var_da = xr.apply_ufunc(
                ...
                input_core_dims=[['station_id']],       # Function operates on the 'station_id' dimension per time slice
                output_core_dims=[['latitude', 'longitude']], # Output has new 'latitude', 'longitude' dimensions
                exclude_dims={'station_id'},            # 'station_id' is an internal detail, excluded from output dims
                dask='parallelized',                    # Enable Dask for parallel execution
                output_dtypes=[np.float32],             # Specify the output data type (float for numerical data)
            ...
            ).chunk({'time': output_grid_chunk_shape[0], # Rechunk the Dask array after creation
    ...
    for var_name in all_zarr_schema_vars:
        if var_name not in gridded_dwd_ds.data_vars and var_name != "time":
            ...
            dtype_for_placeholder = object if var_name == "PTYPE_surface" else np.float32 # PTYPE is the only string variable in schema
...
if __name__ == "__main__":
    ...
    gfs_zarr_reference_path = os.getenv("GFS_ZARR_PATH", "/path/to/your/GFS.zarr") # TODO: Set this environment variable!
    ...
    if saveType == "S3":
        s3_bucket_name = os.getenv("s3_bucket", "your-s3-bucket") # Ensure this env var is set
    ...
    if not dwd_ds_point.dims: # Check if the resulting dataset has dimensions (i.e., is not empty)
    ...
    station_metadata_df.to_json(station_metadata_path, orient="index") # Saves as a dictionary-like JSON (station_id as key)
    ...
    final_gridded_zarr_target_path = os.path.join(forecast_path, "DWD.zarr") # Standardized name for API consumption
    final_time_pickle_path_dest = os.path.join(forecast_path, "DWD.time.pickle")

    if saveType == "S3":
        s3_bucket_name = os.getenv("s3_bucket", "your-s3-bucket") # Ensure this env var is set
        s3_gridded_zarr_key = os.path.join("ForecastTar_v2", "DWD.zarr") # Example S3 key path for gridded Zarr
        s3_time_pickle_key_upload = os.path.join("ForecastTar_v2", "DWD.time.pickle") # S3 key for time pickle
        s3_station_metadata_key = os.path.join("ForecastTar_v2", "DWD_Station_Metadata.json") # S3 key for station metadata
