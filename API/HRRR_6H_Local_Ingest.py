...
process_chunk = CHUNK_SIZES["HRRR_6H"]

# Define the final x/y chunksize
final_chunk = FINAL_CHUNK_SIZES["HRRR_6H"]

his_period = HISTORY_PERIODS["HRRR_6H"]
...
if save_type == "S3":
    ...
    if s3.exists(forecast_path + "/" + ingest_version + "/HRRR_6H.time.pickle"):
        with s3.open(
            forecast_path + "/" + ingest_version + "/HRRR_6H.time.pickle", "rb"
        ) as f:
    ...
    if os.path.exists(forecast_path + "/" + ingest_version + "/HRRR_6H.time.pickle"):
...
matchstring_su = (
    ":((CRAIN|CICEP|CSNOW|CFRZR|PRATE|VIS|GUST|DSWRF):surface:.*hour fcst)"
)
...
if save_type == "S3":
    ...
    s3.put_file(
        ...
        forecast_path + "/" + ingest_version + "/HRRR_6H.zarr.zip",
    )
    ...
    s3.put_file(
        ...
        forecast_path + "/" + ingest_version + "/HRRR_6H.time.pickle",
    )
else:
    # Write most recent forecast time
    with open(forecast_process_dir + "/HRRR_6H.time.pickle", "wb") as file:
    ...
    shutil.copytree(
        ...
        forecast_path + "/" + ingest_version + "/HRRR_6H.zarr",
        dirs_exist_ok=True,
    )
