...
def calculate_period_summary_text(
    ...
    if condition_type == "precip" or condition_type == "cloud":
        ...
        # Humidity-based combination removed: do not append low/high-humidity here
        ...
        # Humidity-based combination removed for wind as well
    ...
    return summary_text, wind_condition_combined, vis_condition_combined
...
def calculate_day_text(
    ...
    if (
        ...
        # dry/humid later checks removed
    ...
    selected_final_summaries = []  # Renamed list to indicate it holds full objects
    ...
    for summary_data in sorted_summaries_candidates:
        ...
        elif summary_data["icon"] == "thunderstorm":
            current_c_icon = "thunderstorm"

        # CHANGE: Append the WHOLE dictionary, not just summary_data["text"]
        selected_final_summaries.append(summary_data)

        if len(selected_final_summaries) >= 2:
            break  # Limit to top 2 conditions

    # Final summary text construction
    if len(selected_final_summaries) == 1:
        # Extract the text from the object
        final_constructed_summary = ["sentence", selected_final_summaries[0]["text"]]

    elif len(selected_final_summaries) == 2:
        primary = selected_final_summaries[0]
        secondary = selected_final_summaries[1]
        ...
        final_constructed_summary = [
            "sentence",
            [joiner, primary["text"], secondary["text"]],
        ]
