...
def build_rdaqa_url(run_time, variable):
    ...
    filename = f"{yyyymmdd}T{hh}Z_MSC_RDAQA-Prelim_{variable}_Sfc_RLatLon0.09_PT0H.grib2"
...
if __name__ == "__main__":
    ...
    zarr_array = zarr.create_array(
        store=zarr_store,
        shape=(len(zarr_output_vars), len(time_unix), sample_da.shape[0], sample_da.shape[1]),
    ...
    time_da_broadcasted = da.broadcast_to(time_da, (1, len(time_unix), sample_da.shape[0], sample_da.shape[1]))
    dask_variable_list.append(time_da_broadcasted)
    
    for var in RDAQA_VARS:
        if var in downloaded_datasets:
            var_data = da.from_array(downloaded_datasets[var].values, chunks=(process_chunk, process_chunk))
            var_data = var_data.reshape(1, 1, sample_da.shape[0], sample_da.shape[1])
        else:
            var_data = da.full((1, 1, sample_da.shape[0], sample_da.shape[1]), np.nan, dtype=np.float32)
    ...
    with ProgressBar():
        final_stacked_dask.rechunk((len(zarr_output_vars), 1, final_chunk, final_chunk)).to_zarr(
    ...
    if save_type == "S3":
        ...
        s3.put_file(forecast_process_path + ".zip", os.path.join(forecast_path, ingest_version, "RDAQA.zarr.zip"))
        s3.put_file(pickle_file_path, os.path.join(forecast_path, ingest_version, "RDAQA.time.pickle"))
        logger.info("S3 target deployment complete.")
    else:
        shutil.move(pickle_file_path, os.path.join(forecast_path, ingest_version, "RDAQA.time.pickle"))
        shutil.copytree(zarr_store_path, os.path.join(forecast_path, ingest_version, "RDAQA.zarr"), dirs_exist_ok=True)
