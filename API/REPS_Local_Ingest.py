...
for g in match_strings:
    ...
    if len(FH.file_exists) != len(reps_file_range):
        print(
            f"Download failed for {g['variable']}:{g['level']}, expected "
            f"{len(reps_file_range)} files but got {len(FH.file_exists)}"
        )
        sys.exit(1)
...
expected_total = len(reps_file_range) * len(match_strings)
...
assert len(xarray_forecast_merged.time) == len(reps_file_range), (
...
for i in range(his_period, 0, -6):
    ...
    # Create a range of dates for historic data going back 72 hours
    DATES = pd.date_range(
