# %% RRFS 6-hourly Processing script using Dask, FastHerbie, and wgrib2
...
forecast_process_dir = os.getenv(
    "forecast_process_dir", default="/home/ubuntu/Weather/RRFS_6H"
)
forecast_process_path = forecast_process_dir + "/RRFS_6H_Process"
tmp_dir = forecast_process_dir + "/Downloads"

forecast_path = os.getenv("forecast_path", default="/home/ubuntu/Weather/Prod/RRFS_6H")
...
process_chunk = CHUNK_SIZES["RRFS_6H"]

# Define the final x/y chunksize
final_chunk = FINAL_CHUNK_SIZES["RRFS_6H"]

his_period = HISTORY_PERIODS["RRFS_6H"]
...
if save_type == "S3":
    ...
    if s3.exists(forecast_path + "/" + ingest_version + "/RRFS_6H.time.pickle"):
        with s3.open(
            forecast_path + "/" + ingest_version + "/RRFS_6H.time.pickle", "rb"
        ) as f:
            previous_base_time = pickle.load(f)

        # Compare timestamps and download if the S3 object is more recent
        if previous_base_time >= base_time:
            logger.info("No Update to RRFS_6H, ending")
            sys.exit()

else:
    if os.path.exists(forecast_path + "/" + ingest_version + "/RRFS_6H.time.pickle"):
...
rrfs_range1 = FORECAST_LEAD_RANGES["RRFS_6H"]
...
xarray_forecast_merged = xarray_forecast_merged.chunk(
    chunks={"time": 31, "x": process_chunk, "y": process_chunk}
)
...
if save_type == "S3":
    zarr_store = zarr.storage.ZipStore(
        forecast_process_dir + "/RRFS_6H.zarr.zip", mode="a", compression=0
    )
else:
    zarr_store = zarr.storage.LocalStore(forecast_process_dir + "/RRFS_6H.zarr")
...
zarr_array = zarr.create_array(
    ...
    chunks=(len(zarr_vars), len(npCatTimes), final_chunk, final_chunk),
...
with dask.config.set(scheduler="threads", num_workers=zarr_store_workers):
    da.rechunk(
        ...
        (len(zarr_vars), len(npCatTimes), final_chunk, final_chunk),
...
if save_type == "S3":
    # Upload to S3
    s3.put_file(
        forecast_process_dir + "/RRFS_6H.zarr.zip",
        forecast_path + "/" + ingest_version + "/RRFS_6H.zarr.zip",
    )

    # Write most recent forecast time
    with open(forecast_process_dir + "/RRFS_6H.time.pickle", "wb") as file:
        # Serialize and write the variable to the file
        pickle.dump(base_time, file)

    s3.put_file(
        forecast_process_dir + "/RRFS_6H.time.pickle",
        forecast_path + "/" + ingest_version + "/RRFS_6H.time.pickle",
    )
else:
    # Write most recent forecast time
    with open(forecast_process_dir + "/RRFS_6H.time.pickle", "wb") as file:
    ...
    shutil.copytree(
        forecast_process_dir + "/RRFS_6H.zarr",
        forecast_path + "/" + ingest_version + "/RRFS_6H.zarr",
        dirs_exist_ok=True,
    )
