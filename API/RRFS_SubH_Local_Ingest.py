# %% RRFS subhourly Processing script using Dask, FastHerbie, and wgrib2
...
if save_type == "S3":
    ...
    if s3.exists(forecast_path + "/" + ingest_version + "/SubH_v2.time.pickle"):
        ...
        if previous_base_time >= base_time:
            logger.info("No Update to SubH, ending")
            sys.exit()
...
if len(xarray_forecast_merged.time) != len(rrfs_range1) * 4:
    logger.info(
        "len(xarray_forecast_merged.time) = %d", len(xarray_forecast_merged.time)
    )
    logger.info("len(rrfs_range1) * 4 = %d", len(rrfs_range1) * 4)

    assert len(xarray_forecast_merged.time) == len(rrfs_range1) * 4, (
