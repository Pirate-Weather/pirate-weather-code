...
# --- Determine the latest run from the S3 bucket's timestamp ---
...
time_str = base_time.strftime("t%H%Mz")
s3_path = f"rtma2p5_ru.{date_str}/rtma2p5_ru.{time_str}.2dvaranl_ndfd.grb2"
local_path = os.path.join(tmp_dir, f"rtma_ru_{date_str}_{time_str}.grb2")

try:
    logging.info(f"Attempting to download {s3_path} from S3 bucket {aws_bucket}")
    s3.get(f"{aws_bucket}/{s3_path}", local_path)
    logging.info("RTMA-RU GRIB file downloaded successfully.")
except FileNotFoundError:
    logging.error(f"Download failed: The file {s3_path} does not exist.")
    sys.exit(1)
except Exception as e:
    logging.error(f"Download failed due to an error: {e}")
    sys.exit(1)

# Create XArray from the downloaded file
grib_list = [local_path]
