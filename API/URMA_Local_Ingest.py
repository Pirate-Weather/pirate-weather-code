...
analysis_process_dir = os.getenv(
    "forecast_process_dir", default="/home/ubuntu/Weather/URMA"
)
analysis_process_path = analysis_process_dir + "/URMA_Process"
tmp_dir = analysis_process_dir + "/Downloads"
analysis_path = os.getenv("forecast_path", default="/home/ubuntu/Weather/Prod/URMA")
historic_base_path = os.getenv(
    "historic_base_path", default="/home/ubuntu/Weather/Hist/URMA"
)

# Define the processing and final chunk size
# NOTE: Using RTMA chunk size here, assuming this is intended based on original script
...
logging.info(f"Checking for new URMA data for base time: {base_time}")

# Check if this is newer than the current file
if save_type == "S3":
    if s3.exists(analysis_path + "/" + ingest_version + "/URMA.time.pickle"):
        with s3.open(
            analysis_path + "/" + ingest_version + "/URMA.time.pickle", "rb"
        ) as f:
            previous_base_time = pickle.load(f)
        if previous_base_time >= base_time:
            logging.info("No Update to URMA, ending")
            sys.exit()

else:
    if os.path.exists(analysis_path + "/" + ingest_version + "/URMA.time.pickle"):
        with open(
            analysis_path + "/" + ingest_version + "/URMA.time.pickle", "rb"
        ) as file:
...
if len(fh_analysis.file_exists) != 1:
    ...
    sys.exit(1)
logging.info("URMA GRIB file downloaded successfully.")
...
logging.info("URMA dataset loaded with xarray.")
...
if save_type == "S3":
    zarr_store = zarr.storage.ZipStore(
        analysis_process_dir + "/URMA.zarr.zip", mode="a", compression=0
    )
else:
    zarr_store = zarr.storage.LocalStore(analysis_process_dir + "/URMA.zarr")
...
if save_type == "S3":
    s3.put_file(
        analysis_process_dir + "/URMA.zarr.zip",
        analysis_path + "/" + ingest_version + "/URMA.zarr.zip",
    )
    logging.info("Final Zarr zip file uploaded to S3.")

    with open(analysis_process_dir + "/URMA.time.pickle", "wb") as file:
        pickle.dump(base_time, file)
    s3.put_file(
        analysis_process_dir + "/URMA.time.pickle",
        analysis_path + "/" + ingest_version + "/URMA.time.pickle",
    )
    ...
    shutil.copytree(
        analysis_process_dir + "/URMA.zarr",
        analysis_path + "/" + ingest_version + "/URMA.zarr",
        dirs_exist_ok=True,
    )
...
if save_type == "S3":
    ...
    s3.put_file(
        analysis_process_dir + "/URMA.zarr.zip",
        historic_save_path + "/URMA.zarr.zip",
    )
    ...
    s3.put_file(
        analysis_process_dir + "/URMA.time.pickle",
        historic_save_path + "/URMA.time.pickle",
    )
