...
def calculate_nowcast_concentration(
    ...
    """Backward-compatible wrapper for AQ nowcast helper in api_utils."""
    return api_utils.calculate_nowcast_concentration(
        concentrations=concentrations, num_hours=num_hours
    )


def trailing_mean(conc: Optional[np.ndarray], window: int) -> Optional[np.ndarray]:
    """Backward-compatible wrapper for AQ trailing-mean helper in api_utils."""
    return api_utils.trailing_mean(conc=conc, window=window)
...
def calculate_aqi(
    ...
    """Backward-compatible wrapper for AQI helper in api_utils."""
    return api_utils.calculate_aqi(
        pm25=pm25,
        pm10=pm10,
        o3=o3,
        no2=no2,
