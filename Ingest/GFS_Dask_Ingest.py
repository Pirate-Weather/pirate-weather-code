...
for i in range(hisPeriod, 0, -6):
    ...
    xarray_hist_merged.to_zarr(
            store=s3store, mode='w', consolidated=True, encoding=encoding)
...
source = zarr.open('/tmp/GFS_UnChunk.zarr')
intermediate = "/tmp/GFS_Mid.zarr"
target = zarr.ZipStore('/tmp/GFS.zarr.zip', compression=0)
...
# Upload to S3
s3.put_file('/tmp/GFS.zarr.zip', s3_bucket + '/ForecastTar/GFS.zarr.zip')
