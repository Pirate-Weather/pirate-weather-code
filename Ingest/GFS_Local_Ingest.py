...
forecast_process_path = os.getenv('forecast_process_path', default='/home/ubuntu/data/GFS_forecast')
hist_process_path     = os.getenv('hist_process_path', default='/home/ubuntu/data/GFS_historic')
...
latestRun = HerbieLatest(model="gfs", n=3, freq="6H", fxx=240,
                               product="pgrb2.0p25", verbose=False, priority='aws')
...
zarrVars = ('time', 'VIS_surface', 'GUST_surface', 'PRMSL_meansealevel', 'TMP_2maboveground', 'DPT_2maboveground',
...
matchStrings = (matchstring_2m + "|" + matchstring_su + "|" + matchstring_10m + "|" +
               matchstring_oz + "|" + matchstring_cl + "|" + matchstring_ap + "|" + matchstring_sl)
...
FH_forecastsub = FastHerbie(pd.date_range(start=base_time, periods=1, freq='6H'),
                               model="gfs", fxx=gfsFileRange,
                               product="pgrb2.0p25", verbose=False, priority='aws', save_dir=tmpDIR)
...
cmd = 'cat '  + ' '.join(gribList) + ' | ' + f"{wgrib2_path}" + ' - ' + ' -netcdf ' + forecast_process_path + '_wgrib2_merged.nc'

# Run wgrib2
spOUT = subprocess.run(cmd, shell=True,  capture_output=True, encoding="utf-8")

#%% Download and add UV data from the pgrib2b product
FH_forecastUV = FastHerbie(pd.date_range(start=base_time, periods=1, freq='6H'),
                            model="gfs", fxx=gfsFileRange, product="pgrb2b.0p25",
...
# Create list of downloaded grib files
gribListUV = [str(Path(x.get_localFilePath(UVmatchString)).expand()) for x in FH_forecastUV.file_exists]

# Create a string to pass to wgrib2 to merge all gribs into one netcdf
cmd = 'cat '  + ' '.join(gribListUV) + ' | ' + f"{wgrib2_path}" + ' - ' + ' -netcdf ' + forecast_process_path + '_wgrib_merged_UV.nc'
...
xarray_forecast_merged = xr.merge([xarray_wgrib_merged, xarray_wgribUV_merged], compat='override')

assert len(xarray_forecast_merged.time) ==  len(gfsFileRange), "Incorrect number of timesteps! Exiting"
...
stacked_times = np.concatenate((pd.date_range(start=start - pd.Timedelta(hisPeriod, 'H'),
                                              end=start - pd.Timedelta(1, 'H'), freq='H'),
...
APCP_surface_tmp = xarray_forecast_merged['APCP_surface'].copy(data=np.diff(xarray_forecast_merged['APCP_surface'],
...
xarray_forecast_distance = xarray_forecast_distance.assign_coords({'time':xarray_forecast_merged.time.data,
...
xarray_forecast_distance['APCP_surface'] = xarray_forecast_distance['APCP_surface'].where(
                                            xarray_forecast_distance['APCP_surface']>0.2,0)
...
for t in range(0, 160):
    distances.append(dask.delayed(proximity)(xarray_forecast_distance['APCP_surface'].isel(time=t), distance_metric="GREAT_CIRCLE",
                                            x='longitude', y='latitude'))

    directions.append(dask.delayed(direction)(xarray_forecast_distance['APCP_surface'].isel(time=t), distance_metric="GREAT_CIRCLE",
                                            x='longitude', y='latitude'))
...
#with ProgressBar():
distanced_chunked.to_zarr(forecast_process_path + '_stormDist.zarr', overwrite=True).compute()
directions_chunked.to_zarr(forecast_process_path + '_stormDir.zarr', overwrite=True).compute()
...
uvProc = xarray_forecast_merged['DUVB_surface'].isel(time=slice(0, 120)).values.reshape(20, 6, 721, 1440, order='C')
...
uvProcHour = np.concatenate((first_step, np.diff(uvProc,
                                      axis=1) * n[:, 1:, :, :] + \
                                      uvProc[:, 0:5, :, :]), axis=1)
...
uvProc = xarray_forecast_merged['DUVB_surface'].isel(time=slice(120, 160)).values.reshape(20, 2, 721, 1440, order='C')
...
del uvProc, uvProcHour, uvProcHour3D, uvProcHour3DB, n, first_step, xarray_wgrib_merged, xarray_wgribUV_merged
...
encoding = {vname: {'compressor': compressor, 'filters': filters} for vname in zarrVars[1:-2]}
...
for i in range(hisPeriod, 0, -6):
    ...
    if saveType=='S3':
      #S3 Path Setup
      s3_path = s3_bucket + '/GFS/GFS_Hist' + (base_time - pd.Timedelta(hours = i)).strftime('%Y%m%dT%H%M%SZ') + '.zarr'
      ...
      #Local Path Setup
      local_path = s3_bucket + '/GFS/GFS_Hist' + (base_time - pd.Timedelta(hours = i)).strftime('%Y%m%dT%H%M%SZ') + '.zarr'
    ...
    # Create FastHerbie Object.
    FH_histsub = FastHerbie(DATES, model="gfs", fxx=fxx, product="pgrb2.0p25", verbose=False)

    # Download the subsets
    FH_histsub.download(matchStrings, verbose=False)

    # Create list of downloaded grib files
    gribList = [str(Path(x.get_localFilePath(matchStrings)).expand()) for x in FH_histsub.file_exists]

    # Create a string to pass to wgrib2 to merge all gribs into one netcdf
    cmd = 'cat '  + ' '.join(gribList) + ' | ' + f"{wgrib2_path}" + ' - ' + ' -netcdf ' + hist_process_path + '_wgrib2_merged.nc'
    ...
    #%% Download and add UV data from the pgrib2b product
    FH_histsubUV = FastHerbie(DATES, model="gfs", fxx=fxx, product="pgrb2b.0p25", verbose=False)

    # Download the subsets
    FH_histsubUV.download(UVmatchString, verbose=False)

    # Create list of downloaded grib files
    gribListUV = [str(Path(x.get_localFilePath(UVmatchString)).expand()) for x in FH_histsubUV.file_exists]

    # Create a string to pass to wgrib2 to merge all gribs into one netcdf
    cmd = 'cat '  + ' '.join(gribListUV) + ' | ' + f"{wgrib2_path}" + ' - ' + ' -netcdf ' + hist_process_path + '_wgrib2_merged_UV.nc'
    ...
    xarray_his_wgribUV_merged = xr.open_dataset(hist_process_path + '_wgrib2_merged_UV.nc')

    xarray_hist_merged = xr.merge([xarray_his_wgrib_merged, xarray_his_wgribUV_merged], compat='override')
    ...
    xarray_hist_distance = xarray_hist_distance.assign_coords({'time': xarray_hist_merged.time.data,
                                                                       'latitude': xarray_hist_merged.latitude.data,
                                                                       'longitude': ((
                                                                            xarray_hist_merged.longitude + 180) % 360) - 180})
    ...
    # Find nearest storm distance and direction for first 12 hours
    for t in range(0, 6):
        distances.append(dask.delayed(proximity)(xarray_hist_distance['APCP_surface'].isel(time=t),
                                                 distance_metric="GREAT_CIRCLE",
                                                 x='longitude', y='latitude'))

        directions.append(dask.delayed(direction)(xarray_hist_distance['APCP_surface'].isel(time=t),
                                                  distance_metric="GREAT_CIRCLE",
                                                  x='longitude', y='latitude'))

    # Copy back to main array
    #with ProgressBar():
    xarray_hist_merged['Storm_Distance'] = (('time', 'latitude', 'longitude'), dask.delayed(da.stack)(distances).rechunk((6,100,100)).compute())
    xarray_hist_merged['Storm_Direction'] = (('time', 'latitude', 'longitude'), dask.delayed(da.stack)(directions).rechunk((6,100,100)).compute())
    ...
    # Clear memory
    del uvProc, uvProcHour, apcpProc, apcpProcHour,  xarray_his_wgrib_merged, xarray_his_wgribUV_merged
    ...
    #No chunking since only one time step
    encoding = {vname: {'compressor': compressor, 'filters': filters, 'chunks': (6, 100, 100)} for vname in zarrVars[1:-2]}
...
zarr_array = zarr.zeros((21, 276, 721, 1440), chunks=(1, 276, 20, 20), store=zarr_store, compressor=compressor, dtype='float32')

#%% Merge the historic and forecast datasets and then squash using dask
# Get the s3 paths to the historic data
ncLocalWorking_paths = [s3_bucket + '/GFS/GFS_Hist' + (base_time - pd.Timedelta(hours = i)).strftime('%Y%m%dT%H%M%SZ')+ '.zarr' for i in range(hisPeriod, 0, -6)]
...
for daskVarIDX, dask_var in enumerate(zarrVars):
    for local_ncpath in ncLocalWorking_paths:
        ...
        if saveType=='S3':
            daskVarArrays.append(da.from_zarr(
                local_ncpath, component=dask_var, inline_array = True, storage_options={'key': 'AKIA2HTALZ5LWRCTHC5F',
                                                    'secret': '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'}))
        else:
    ...
    if dask_var == 'Storm_Distance':
        ...
        daskForecastArray = da.from_zarr(ncForecastWorking_path + '_.zarr', component=dask_var, inline_array = True)
    ...
    if dask_var == 'time':
        ...
        # with ProgressBar():
        interpTimes = da.map_blocks(linInterp, daskCatTimes.rechunk((196)),
        ...
        da.to_zarr(da.from_array(np.expand_dims(
            ...
            region=(slice(daskVarIDX, daskVarIDX + 1), slice(0, 276), slice(0, 721), slice(0, 1440)))

    else:
        daskVarArraysShape = da.reshape(daskVarArraysStack, (36, 721, 1440), merge_chunks=False)
        daskArrayOut = da.concatenate((daskVarArraysShape, daskForecastArray), axis=0)

        # with ProgressBar():
        da.to_zarr(da.from_array(da.expand_dims(
            da.map_blocks(linInterp, daskArrayOut[:, :, :].rechunk((196, 20, 20)).astype('float32'),
...
rechunked = rechunk(source, target_chunks=(21, 276, 1, 1), target_store=target,
                    max_mem='1G',
...
if saveType=='S3':
    # Upload to S3
    s3.put_file(merge_process_dir + '/GFS.zarr.zip', s3_bucket + '/ForecastTar/GFS.zarr.zip')
    ...
    s3.put_file(merge_process_dir + '/GFS.time.pickle', s3_bucket + '/ForecastTar/GFS.time.pickle') 
        
else:
    # Move to local
    shutil.move(merge_process_dir + '/GFS.zarr.zip', s3_bucket + '/ForecastTar/GFS.zarr.zip')
    ...
    shutil.move(merge_process_dir + '/GFS.time.pickle', s3_bucket + '/ForecastTar/GFS.time.pickle')       
