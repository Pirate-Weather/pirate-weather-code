...
forecast_process_path = os.getenv('forecast_process_path', default='/home/ubuntu/data/HRRR_forecast')
...
latestRun = Herbie_latest(model="hrrr", n=3, freq="6H", fxx=[48],
...
xarray_forecast_merged = xarray_forecast_merged.chunk(chunks={'time': 31, 'x': 90, 'y': 90})
...
if saveType == 'S3':
    # Upload to S3
    s3.put_file(merge_process_dir + '/HRRR_6H.zarr.zip', s3_bucket + '/ForecastTar/HRRR_6H.zarr.zip')
    ...
    s3.put_file(merge_process_dir + '/HRRR_6H.time.pickle', s3_bucket + '/ForecastTar/HRRR_6H.time.pickle')             
    
else:
    # Move to local
    shutil.move(merge_process_dir + '/HRRR_6H.zarr.zip', s3_bucket + '/ForecastTar/HRRR_6H.zarr.zip')
    # Write most recent forecast time
    with open(merge_process_dir + '/HRRR_6H.time.pickle', 'wb') as file:
      # Serialize and write the variable to the file
      pickle.dump(base_time, file)
      
    shutil.move(merge_process_dir + '/HRRR_6H.time.pickle', s3_bucket + '/ForecastTar/HRRR_6H.time.pickle')  
    # Clean up
