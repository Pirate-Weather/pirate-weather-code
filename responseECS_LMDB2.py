...
def download_if_newer(s3_bucket, s3_object_key, local_file_path, local_lmdb_path, lmdb_map):

    # Initialize the S3 client
    s3_client = boto3.client('s3',aws_access_key_id="AKIA2HTALZ5LWRCTHC5F",
    ...
    if newFile == True:
      ...
      lmdb_store = zarr.LMDBStore(local_lmdb_path, map_size=lmdb_map, sync=False, map_async=False, writemap=False)
...
if STAGE != "TESTING":
  download_if_newer(s3_bucket, 'ForecastTar/SubH.zarr.zip', '/tmp/SubH.zarr.zip', '/tmp/SubH.zarr.mdb', 2000000000)
  print('SubH Initial Download!')
  download_if_newer(s3_bucket, 'ForecastTar/HRRR_6H.zarr.zip', '/tmp/HRRR_6H.zarr.zip', '/tmp/HRRR_6H.zarr.mdb', 3000000000)
  print('HRRR_6H Initial Download!')
  download_if_newer(s3_bucket, 'ForecastTar/GFS.zarr.zip', '/tmp/GFS.zarr.zip', '/tmp/GFS.zarr.mdb', 20000000000)
  print('GFS Initial Download!')
  download_if_newer(s3_bucket, 'ForecastTar/NBM.zarr.zip', '/tmp/NBM.zarr.zip', '/tmp/NBM.zarr.mdb', 25000000000)
  print('NBM Initial Download!')
  download_if_newer(s3_bucket, 'ForecastTar/NBM_Fire.zarr.zip', '/tmp/NBM_Fire.zarr.zip', '/tmp/NBM_Fire.zarr.mdb', 5000000000)
  print('NBM Initial Download!')  
  download_if_newer(s3_bucket, 'ForecastTar/GEFS.zarr.zip', '/tmp/GEFS.zarr.zip', '/tmp/GEFS.zarr.mdb', 5000000000)
  print('GEFS Initial Download!')
  download_if_newer(s3_bucket, 'ForecastTar/HRRR.zarr.zip', '/tmp/HRRR.zarr.zip', '/tmp/HRRR.zarr.mdb', 7000000000)
  print('HRRR Initial Download!')  
  download_if_newer(s3_bucket, 'ForecastTar/ETOPO_DA_C.zarr.zip', '/tmp/ETOPO_DA_C.zarr.zip', '/tmp/ETOPO_DA_C.zarr.mdb', 2000000000)
  print('ETOPO Initial Download!')
  download_if_newer(s3_bucket, 'ForecastTar/NWS_Alerts.zarr.zip', '/tmp/NWS_Alerts.zarr.zip', '/tmp/NWS_Alerts.zarr.mdb', 1000000000)
...
# Initial Open
SubH_lmdb_ReadStore = zarr.LMDBStore('/tmp/SubH.zarr.mdb', readonly=True, buffers=True, lock=False, map_async=False, readahead=False, writemap=False, sync=True)
SubH_Zarr = zarr.open(SubH_lmdb_ReadStore, mode='r')

HRRR_6H_lmdb_ReadStore = zarr.LMDBStore('/tmp/HRRR_6H.zarr.mdb', readonly=True, buffers=True, lock=False, map_async=False, readahead=False, writemap=False, sync=True)
HRRR_6H_Zarr = zarr.open(HRRR_6H_lmdb_ReadStore, mode='r')

GFS_lmdb_ReadStore = zarr.LMDBStore('/tmp/GFS.zarr.mdb', readonly=True, buffers=True, lock=False, map_async=False, readahead=False, writemap=False, sync=True)
GFS_Zarr = zarr.open(GFS_lmdb_ReadStore, mode='r')

NBM_lmdb_ReadStore = zarr.LMDBStore('/tmp/NBM.zarr.mdb', readonly=True, buffers=True, lock=False, map_async=False, readahead=False, writemap=False, sync=True)
NBM_Zarr = zarr.open(NBM_lmdb_ReadStore, mode='r')

NBM_Fire_lmdb_ReadStore = zarr.LMDBStore('/tmp/NBM_Fire.zarr.mdb', readonly=True, buffers=False, lock=False, map_async=False, readahead=False, writemap=False, sync=True)
NBM_Fire_Zarr = zarr.open(NBM_Fire_lmdb_ReadStore, mode='r')

GEFS_lmdb_ReadStore =zarr.LMDBStore('/tmp/GEFS.zarr.mdb', readonly=True, buffers=True, lock=False, map_async=False, readahead=False, writemap=False, sync=True)
GEFS_Zarr = zarr.open(GEFS_lmdb_ReadStore, mode='r')

HRRR_lmdb_ReadStore =  zarr.LMDBStore('/tmp/HRRR.zarr.mdb', readonly=True, buffers=True, lock=False, map_async=False, readahead=False, writemap=False, sync=True)
HRRR_Zarr = zarr.open(HRRR_lmdb_ReadStore, mode='r')

ETOPO_lmdb_ReadStore = zarr.LMDBStore('/tmp/ETOPO_DA_C.zarr.mdb', readonly=True, buffers=True, lock=False, map_async=False, readahead=False, writemap=False, sync=True)
ETOPO_f = zarr.open(ETOPO_lmdb_ReadStore, mode='r')

NWS_Alerts_ReadStore =  zarr.LMDBStore('/tmp/NWS_Alerts.zarr.mdb', readonly=True, buffers=True, lock=False, map_async=False, readahead=False, writemap=False, sync=True)
...
def PW_Forecast(location: str,
    ...
    if az_Lon < -134 or az_Lon > -61 or lat < 21 or lat > 53 or exHRRR == 1:
        ...
        if ((dataOut is not False) and (dataOut_h2 is not False) and (dataOut_hrrrh is not False)):
            ...
            if (utcTime - datetime.datetime.utcfromtimestamp(subhRunTime.astype(int))) > datetime.timedelta(hours=4):
                dataOut = False
                print('OLD SubH')

            hrrrhRunTime = dataOut_hrrrh[36, 0]
            #print( datetime.datetime.utcfromtimestamp(dataOut_hrrrh[35, 0].astype(int))) 
            if (utcTime - datetime.datetime.utcfromtimestamp(hrrrhRunTime.astype(int))) > datetime.timedelta(hours=16):
                dataOut_hrrrh = False
                print('OLD HRRRH')


            h2RunTime = dataOut_h2[0, 0]
            if (utcTime - datetime.datetime.utcfromtimestamp(h2RunTime.astype(int))) > datetime.timedelta(hours=46):
                dataOut_h2 = False
                print('OLD HRRR_6H')
    ...
    sourceTimes['gfs'] = rounder(datetime.datetime.utcfromtimestamp(gfsRunTime.astype(int))).strftime('%Y-%m-%d %HZ')
    sourceTimes['gefs'] = rounder(datetime.datetime.utcfromtimestamp(gefsRunTime.astype(int))).strftime('%Y-%m-%d %HZ')
    ...
    baseTime = utc.localize(datetime.datetime(year=utcTime.year, month=utcTime.month, day=utcTime.day, hour=utcTime.hour,
                                 minute=utcTime.minute)).astimezone(pytzTZ)
    baseHour = pytzTZ.localize(datetime.datetime(year=baseTime.year, month=baseTime.month, day=baseTime.day, hour=baseTime.hour))                                 
               
    baseDay = baseTime.replace(hour=0, minute=0, second=0, microsecond=0)
    
  
    # Find UTC time for the base day
    baseDayUTC_Grib = (np.datetime64(baseDay.astimezone(utc)) - np.datetime64(datetime.datetime(1970, 1, 1, 0, 0, 0))).astype(
    ...
    try: # Add a fallback to GFS if these don't work
      ...
      if 'nbm_fire' in sourceList:   
          ...
          NBM_Fire_Merged[0:(217 - NBM_Fire_StartIDX), :] = dataOut_nbmFire[NBM_Fire_StartIDX:(numHours + NBM_Fire_StartIDX), :]
    except:
    ...
    ### Minutely
    minute_array = np.arange(baseTime.astimezone(utc), baseTime + datetime.timedelta(minutes=61), datetime.timedelta(minutes=1))
    ...
    hour_array = np.arange(baseDay.astimezone(utc), baseDay.astimezone(utc) + datetime.timedelta(days=8) + datetime.timedelta(hours=1),
    ...
    day_array_4am_grib =  np.array([pytzTZ.localize(datetime.datetime(year=baseTime.year,
                              month=baseTime.month, day=baseTime.day, hour=4) + datetime.timedelta(days=i)).astimezone(utc).timestamp() for i in range(9)]).astype(np.int32)    
    
    day_array_6am_grib =  np.array([pytzTZ.localize(datetime.datetime(year=baseTime.year,
                              month=baseTime.month, day=baseTime.day, hour=6) + datetime.timedelta(days=i)).astimezone(utc).timestamp() for i in range(9)]).astype(np.int32)    
    
    day_array_6pm_grib =  np.array([pytzTZ.localize(datetime.datetime(year=baseTime.year,
                              month=baseTime.month, day=baseTime.day, hour=18) + datetime.timedelta(days=i)).astimezone(utc).timestamp() for i in range(9)]).astype(np.int32)      
...
#        'timedelta64[s]').astype(np.int32)
    ...
    for d in range(0, 8):
        ...
        hourlyDay4amIndex[np.where((hour_array_grib >= day_array_4am_grib[d]) & (hour_array_grib < day_array_4am_grib[d + 1]))] = d
        hourlyHighIndex[
    ...
    InterPhour[:, 15] = np.minimum(np.choose(np.argmin(np.isnan(VisibilityHour), axis=1), VisibilityHour.T), 16090) * visUnits
    ...
    InterPhour[:, 17] = np.maximum(np.choose(np.argmin(np.isnan(PrecpAccumHour), axis=1), PrecpAccumHour.T) * prepAccumUnit,0)
    ...
    InterPcurrent[5] = ((InterPhour[:, 5] - 273.15) + 0.33 *  InterPhour[:, 8] * \
                        6.105 * np.exp(17.27 * (InterPhour[:, 5] - 273.15) / (237.7 + (InterPhour[:, 5] - 273.15))) - 0.70 * \
    ...
    for mask in masks:
        ...
        if dailyTypeCount[0][0] == 0:
          if len(dailyTypeCount) == 2:
            ...
            maxPchanceDay.append(dailyTypeCount[0][0]) # If all ptypes are none, then really shouldn't be any precipitation    
    ...
    for idx in range(0, 8):
        ...
        if InterPdayMax4am[idx, 3] > 0.3 and (((InterPdaySum4am[idx, 21] + InterPdaySum4am[idx, 23]) > (1 * prepAccumUnit)) or (InterPdaySum4am[idx, 22] > (10 * prepAccumUnit))):
    ...
    # try:
    if ((exAlerts == 0) and (az_Lon > -127) and (az_Lon < -65) and (lat > 24) and (lat < 50)):
    ...
    if 'hrrrsubh' in sourceList:
        InterPcurrent[8] = hrrrSubHInterpolation[0, 2] * pressUnits
    else:
    ...
    InterPcurrent[5] = ((InterPcurrent[4] - 273.15) + 0.33 * InterPcurrent[7] * \
                        6.105 * np.exp(17.27 * (InterPcurrent[4] - 273.15) / (237.7 + (InterPcurrent[4] - 273.15))) - 0.70 * \
