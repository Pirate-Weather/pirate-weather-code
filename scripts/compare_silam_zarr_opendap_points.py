#!/usr/bin/env python3
"""Compare random FMI SILAM production Zarr points with OPeNDAP best data.

Default usage from this repo:

    .venvs/ingest-test/bin/python scripts/compare_silam_zarr_opendap_points.py

Or activate the ingest test environment first:

    source .venvs/ingest-test/bin/activate-pirate-ingest
    python scripts/compare_silam_zarr_opendap_points.py
"""
...
DEFAULT_ZARR_PATH = "/home/reya/Weather/Prod/v30/SILAM.zarr"
DEFAULT_OPENDAP_URL = (
    "https://thredds.silam.fmi.fi/thredds/dodsC/"
    "silam_glob_v6_1_sfc/silam_glob_v6_1_sfc_best.ncd.html"
)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Compare random FMI SILAM production Zarr points against the "
    ...
    parser.add_argument(
        ...
        help=f"Production SILAM Zarr path (default: {DEFAULT_ZARR_PATH})",
    )
    parser.add_argument(
        "--opendap-url",
        default=DEFAULT_OPENDAP_URL,
        help="FMI SILAM OPeNDAP URL. A trailing .html is accepted.",
    )
    parser.add_argument(
        "--samples",
        type=int,
    ...
    parser.add_argument(
        ...
        help="Variables to sample (default: all SILAM concentration variables)",
    )
    parser.add_argument(
        "--show-matches",
        action="store_true",
...
def open_production_zarr(path: str) -> zarr.Array:
    ...
    if not isinstance(root, zarr.Array):
        raise TypeError("Expected SILAM production Zarr to be a root zarr.Array")
    if root.ndim != 4 or root.shape[0] < len(ZARR_VARS):
        raise ValueError(
            "Expected SILAM production Zarr shape "
...
def compare_random_points(args: argparse.Namespace) -> int:
    ...
    print("Comparing FMI SILAM production Zarr to OPeNDAP best time series")
