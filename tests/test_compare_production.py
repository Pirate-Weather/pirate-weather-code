...
def test_local_vs_production(lat, lon):
    ...
    local_resp = client.get(
        f"/forecast/{PW_API}/{lat},{lon}?version=2&include=day_night_forecast,aimodels,airqualitydetails&aqiunits=who"
    )
    assert local_resp.status_code == 200
    local_data = local_resp.json()

    prod_url = f"{PROD_BASE}/{PW_API}/{lat},{lon}?version=2&include=day_night_forecast,aimodels,airqualitydetails&aqiunits=who"
    try:
