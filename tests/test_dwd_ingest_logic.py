    ...
    def test_historic_loop_logic(self):
        ...
        def mock_process(df, perform_temporal_interp=True):
            ...
            ds = xr.Dataset(
                ...
                coords={"time": times, "lat": [50.0], "lon": [10.0]},
            )
            return ds
        ...
        with (
            ...
            lazy_open_results = [
                xr.Dataset(
                    {"dummy": (("time"), [1])},
                    coords={"time": [pd.Timestamp("2025-12-10 17:00")]},
                ),
                xr.Dataset(
                    {"dummy": (("time"), [1])},
                    coords={"time": [pd.Timestamp("2025-12-10 18:00")]},
                ),
            ]
            ...
            def mock_lazy_open(store, engine, chunks):
                ...
                return xr.Dataset(
                    {"dummy": (("time"), [1])},
                    coords={"time": [pd.Timestamp("2025-12-10 19:00")]},
                )
            ...
            for i in range(history_period, 0, -1):
                ...
                if not df_hist_filtered.empty:
                    ...
                    with patch("xarray.Dataset.to_zarr"):
