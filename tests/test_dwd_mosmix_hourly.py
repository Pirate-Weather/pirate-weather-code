...
from API.forecast_sources import merge_hourly_models, SourceMetadata
...
def test_dwd_mosmix_hourly_inputs():
    ...
    assert temp_inputs.shape[0] == num_hours, "Temperature inputs should have correct number of hours"
    assert temp_inputs.shape[1] >= 1, "Temperature inputs should have at least one source"
    
    # The first non-NaN value should be from DWD MOSMIX
    first_temp = temp_inputs[0, np.argmin(np.isnan(temp_inputs[0, :]))]
    ...
    first_cloud = cloud_inputs[0, np.argmin(np.isnan(cloud_inputs[0, :]))]
    ...
    first_vis = vis_inputs[0, np.argmin(np.isnan(vis_inputs[0, :]))]
