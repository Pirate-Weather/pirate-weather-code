...
def test_priority_with_all_models_north_america():
    ...
    nbm_merged = np.full((num_hours, 50), np.nan)
    nbm_merged[:, 0] = np.arange(num_hours) * 3600
    nbm_merged[:, 1] = 20.0  # NBM temp (index 1 is temp in NBM model)
    
    hrrr_merged = np.full((num_hours, 50), np.nan)
    hrrr_merged[:, 0] = np.arange(num_hours) * 3600
    hrrr_merged[:, 1] = 18.0  # HRRR temp
    ...
    gfs_merged = np.full((num_hours, 50), np.nan)
    gfs_merged[:, 0] = np.arange(num_hours) * 3600
    gfs_merged[:, 1] = 10.0  # GFS temp
